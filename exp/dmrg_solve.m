function [x]=dmrg_solve(a,y,x0,eps,rmax,nswp)
%[X]=DMRG_SOLVE(A,Y,X0,EPS,RMAX,NSWP)
...
for swp=1:nswp
   ...
   %  x=tt_add(x,tt_scal(x0,sqrt(tt_dot(x0,x0))));
 %x=x0;
...
%N1=rxn1*n1; N2=n2*rxn3; M1=rxm1*m1; M2=m2*rxm3;
 ...
 bprec=[];
%  fprintf('swp=%d,norm_res=%g\n',i,norm(bfun1(sol_prev,a1,a2,ph1,ph3,n1,m1,n2,m2,ra1,ra2,ra3,rxn1,rxm1,rxn3,rxm3)-rhs)/norm(rhs));
  sol_acc = gmres(@(vec)bfun1(vec,a1,a2,ph1,ph3,n1,m1,n2,m2,ra1,ra2,ra3,rxn1,rxm1,rxn3,rxm3),...
      rhs,100,eps,10,bprec,[],sol_prev);
  
  %Now we will simply (???) stabilize f
 ...
 flm=norm(s); %er0=flm.^2; 
  for r=1:size(s,1)
    er0=norm(s(r+1:numel(s)));
      ...
      if (resid< max(eps,2*res_true))%&& er0 < eps*flm) 
          break;
      end;
    %  er0=er0-s(r,r).^2;
  end;
